...
@interface MTCollectedViewController ()
/** 界面数据 */
...
@implementation MTCollectedViewController
...
- (void)deleteButtonDidClicked:(UIBarButtonItem *)buttonItem{
    ...
    [MTDealDataTool unCollectDeals:self.tempDeals];
    self.deals = [MTDealDataTool collectedDeals];
    self.selectedCount = 0;
...
- (NSArray *)deals{
    if (_deals == nil) {
        _deals = [MTDealDataTool collectedDeals];
    }
    return _deals;
}
