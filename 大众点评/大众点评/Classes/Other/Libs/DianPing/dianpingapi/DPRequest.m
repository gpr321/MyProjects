    ...
    if (!result) {
		...
		if ([status isEqualToString:@"OK"]) {
			...
			if ([status isEqualToString:@"ERROR"]) {
				// TODO: 处理错误代码
                NSError *merror = [NSError errorWithDomain:@"参数有误" code:100 userInfo:nil];
                [self failedWithError:merror];
			}
		}
	}
}
...
+ (NSString *)serializeURL:(NSString *)baseURL params:(NSDictionary *)params
{
	...
	NSMutableString *paramsString = [NSMutableString stringWithFormat:@"appkey=%@", kDianPingAppKey];
	...
	[signString appendString:kDianPingSecretKey];
